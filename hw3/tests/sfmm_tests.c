...
Test(sfmm_student_suite, student_test1_internal_frag, .timeout = TEST_TIMEOUT) {
	size_t size_x = 20; size_t size_y = 3; size_t size_z = 10; 
	sf_malloc(size_x);
	sf_malloc(size_y);
	sf_malloc(size_z);
	...
	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0, 1);
	assert_free_block_count(PAGE_SZ-48-96, 1);

	double actual_internal_frag = (double)33/96;
	cr_assert(internal_frag == actual_internal_frag, "Wrong amount of internal fragmentation! Expected (%f) got (%f)!", actual_internal_frag, internal_frag);
}
Test(sfmm_student_suite, student_test2_peak_util, .timeout = TEST_TIMEOUT){
	size_t size_x = 15; size_t size_y = 3; size_t size_z = 10;  size_t size_a = 70;
	sf_malloc(size_x);
	sf_malloc(size_y);
	sf_malloc(size_z);
	void *a = sf_malloc(size_a);

	double peak_util = sf_peak_utilization();
	double actual_peak_util = (double)98/PAGE_SZ;

	assert_quick_list_block_count(0, 0);
	assert_free_block_count(0, 1);
	assert_free_block_count(PAGE_SZ-48-176, 1);
	
	cr_assert(peak_util == actual_peak_util, "Wrong peak utilization value! Expected (%f) got (%f)!", actual_peak_util, peak_util);

	//Then, test if the peak utilization stays the same or not following a realloc to smaller size
	sf_realloc(a, 20);
	peak_util = sf_peak_utilization();
	cr_assert(peak_util == actual_peak_util, "Wrong peak utilization value! Expected (%f) got (%f)!", actual_peak_util, peak_util);
}
Test(sfmm_student_suite, student_test3_flush_quick_list, .timeout = TEST_TIMEOUT){
	size_t size_a = 20; 
	void* blks[10];
	int i;
	...
	//Free one of the odd index blocks to trigger flush
	sf_free(blks[1]);

	assert_quick_list_block_count(0, 1);
	assert_quick_list_block_count(32, 1);
	assert_free_block_count(0, 6);
	assert_free_block_count(PAGE_SZ-48-320, 1);
	assert_free_block_count(32, 5);
}
Test(sfmm_student_suite, student_test4_realloc_as_free, .timeout = TEST_TIMEOUT){
	size_t size_a =  56;
	void* blk = sf_malloc(size_a);
	void* new_blk = sf_realloc(blk, 0);

	cr_assert(!new_blk, "Expected pointer to payload of allocated block to be NULL!");
	assert_quick_list_block_count(0, 1);
	assert_quick_list_block_count(64, 1);
	assert_free_block_count(0,1);
	assert_free_block_count(PAGE_SZ-48-64, 1);
}
Test(sfmm_student_suite, student_test5_invalid_realloc, .timeout = TEST_TIMEOUT){
	size_t size_a =  3;
	void* n = sf_malloc(size_a);

	cr_assert_null(sf_realloc(n+32,0), "Error: expected sf_realloc to return null pointer!");
	cr_assert(errno == EINVAL, "Error: Got (%d) value for errno instead of (%d)", errno, EINVAL);
}
